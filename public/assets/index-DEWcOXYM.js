...
Rules:
- No markdown fences.
- Make the previewHtml runnable and visually close to the app.
- Prefer clean, production-minded code.
- Keep files focused and working.`;
  ...
  return chromeShell(`
    <header class="topbar">
      ...
      <div class="topbar-title">
        <div class="title-main">New App</div>
        <div class="title-sub">Describe the product, then generate a previewable project</div>
      </div>
    </header>
    <main class="page narrow">
      <section class="editor-card">
        <div class="eyebrow">DESCRIBE → GENERATE → PREVIEW</div>
        <h1>Write a concise brief.</h1>
        <p>The generator returns a JSON project with files, setup notes, and a standalone preview. Save it into your browser database.</p>
        <textarea data-field="newPrompt" rows="7" placeholder='Example: "A customer dashboard for invoices, charts, and activity logs."'>${escapeHtml(state.newPrompt)}</textarea>
        <div class="row-between">
          <div class="hint">Press <kbd>Ctrl</kbd> + <kbd>Enter</kbd> to generate</div>
          <button class="primary-btn" data-action="generate-project" ${state.busy ? "disabled" : ""}>${state.busy ? "Generating..." : "Generate App"}</button>
        </div>
        <div class="examples">
          ${[
            "A monochrome task manager with filters and drag-and-drop",
            "A clean portfolio site with case studies and contact form",
            "A finance dashboard with tables, charts, and export",
            "A note app with tags, search, and offline storage",
        ...
        ${state.currentTab === "setup" ? `
          <div class="setup-grid">
            ...
            <section class="setup-block">
              ...
              <div class="setup-list">
                ${(result.nextImprovements || []).map((item) => `
                  <div class="setup-item">

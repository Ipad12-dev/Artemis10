...
function makePreviewRuntime(files, project) {
  const css = files
    ...
    .join("\n\n");
  const scripts = files
    .filter((file) => /\.(js|jsx|ts|tsx)$/.test(file.path))
    .map((file) => `\n/* ${file.path} */\n${stripImportsAndExports(file.content)}`)
    .join("\n\n");
  const appFile = files.find((file) => /(?:^|\/)(app|main|index)\.(jsx|tsx|js|ts)$/i.test(file.path)) || files.find((file) => /\.(jsx|tsx|js|ts)$/.test(file.path));
...
<head>
  ...
  <style>
    ...
    body{font-family:${SYSTEM_FONT};background:#fff;color:#111}
    #preview-error{display:none;margin:24px;padding:18px;border:1px solid #d8d8d8;background:#fafafa;color:#111;border-radius:12px;font-family:${CODE_FONT};white-space:pre-wrap;line-height:1.6}
  </style>
  ${css ? `<style>${css}</style>` : ""}
</head>
<body>
  ...
  <script type="text/babel" data-presets="env,react,typescript">
    ...
    if (Candidate && rootElement) {
      ReactDOM.createRoot(rootElement).render(<Candidate />);
    } else if (rootElement) {
      rootElement.innerHTML = ${JSON.stringify(`
        <main style="min-height:100vh;display:grid;place-items:center;padding:32px;background:#fff;color:#111">
          <section style="max-width:760px;border:1px solid #ddd;padding:24px;border-radius:14px">
            <h1 style="font-family:Georgia,serif;margin:0 0 10px">${escapeHtml(project?.name || "Preview")}</h1>
            <p style="color:#555;line-height:1.7">Artemis could not find an App, Home, LandingPage, or Website component to mount from ${escapeHtml(appFile?.path || "the generated files")}.</p>
          </section>
        </main>
      `)};
    }
...
function getPreviewSrcDoc(project) {
  ...
  const runnableFiles = files.filter((file) => /\.(js|jsx|ts|tsx)$/.test(file.path));
...
<body style="margin:0;font-family:${SYSTEM_FONT};background:#fff;color:#111;display:grid;place-items:center;min-height:100vh;padding:32px">
  <section style="max-width:720px;border:1px solid #ddd;padding:24px;border-radius:14px">
    <h1 style="font-family:${SERIF_FONT};margin:0 0 10px">Preview unavailable</h1>
    <p style="color:#555;line-height:1.7">This project does not include renderable HTML, CSS, or JavaScript files yet. Try refining it with: "make this preview runnable as a React App component."</p>
  </section>
</body>
</html>`;
}
